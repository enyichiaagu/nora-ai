...
const CreateSession = () => {
  ...
  const { error, makeCall, resetCall } = useCall();
  ...
  const handleTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (scheduledDate === today) {
      ...
      if (selectedDateTime < tenMinutesFromNow) {
        showToast.error('Please select a time at least 10 minutes from now');
  ...
  const handleCreateSession = async () => {
    ...
    showToast.message('Loading Conversation');
    ...
    // Reset any previous conversation data before making a new call
    await resetCall();

    try {
      setIsLoading(true);
      if (!profile) {
        showToast.error('Only Authenticated user can create sessions');
      ...
      const status = sessionTiming == 'immediate' ? 'IN_PROGRESS' : 'SCHEDULED';
                ...
                >
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='text-left'>
                      <p className='font-medium text-sm '>Start Now</p>
                ...
                >
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='text-left'>
                      ...
                      <p className='text-xs opacity-70'>Choose date & time</p>
                    </div>
                  </div>
                </button>
              </div>
